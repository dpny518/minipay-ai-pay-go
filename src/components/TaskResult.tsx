...
function ExplorerLink({ hash, chainId }: { hash: string; chainId: number }) {
  ...
  return (
    <a
      ...
      className="text-xs text-[#35D07F] underline break-all"
    >
...
function WebScrapeResult({ data }: { data: unknown }) {
  ...
  return (
    <div className="prose-result text-sm text-zinc-300 leading-relaxed max-h-80 overflow-y-auto">
      <pre className="whitespace-pre-wrap break-words text-xs bg-zinc-900 p-3 rounded-lg border border-zinc-800">
        {markdown}
      </pre>
    </div>
  )
      ...
      <div className="p-3 rounded-lg bg-zinc-900/60 border border-zinc-800 space-y-1.5">
        ...
        <div className="flex gap-4 mt-2">
          {p.follower_count !== undefined && (
            <span className="text-xs text-zinc-300">
              <span className="font-semibold text-white">
                {(p.follower_count as number).toLocaleString()}
              </span>{' '}
              followers
            </span>
          )}
          {p.following_count !== undefined && (
            <span className="text-xs text-zinc-300">
              <span className="font-semibold text-white">
                {(p.following_count as number).toLocaleString()}
              </span>{' '}
              following
            </span>
          )}
...
export function TaskResultCard({ result, txHash, chainId, phase }: Props) {
    ...
    <div className="mx-4 mt-1 rounded-xl border border-zinc-800 bg-zinc-900/40 overflow-hidden">
      ...
      {txHash && (
        <div className="px-4 py-2 border-t border-zinc-800 flex items-center justify-between">
          <span className="text-[10px] text-zinc-600">Tx</span>
